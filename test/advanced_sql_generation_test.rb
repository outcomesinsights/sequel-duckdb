...
class AdvancedSqlGenerationTest < SequelDuckDBTest::TestCase
  ...
  def test_join_with_complex_conditions
    dataset = mock_dataset(:users).join(:profiles, user_id: :id).where(Sequel[:profiles][:active] => true)
  ...
  def test_subquery_in_select
    subquery = mock_dataset(:orders).select(Sequel.function(:count, :*)).where(user_id: Sequel[:users][:id])
  ...
  def test_exists_subquery
    subquery = mock_dataset(:orders).where(user_id: Sequel[:users][:id])
  ...
  def test_scalar_subquery
    subquery = mock_dataset(:orders).select(Sequel.function(:max, :amount)).where(user_id: Sequel[:users][:id])
  ...
  def test_recursive_cte
    ...
    # Since 'n + 1' is a literal, 'n' is not quoted
  ...
  def test_mixed_regular_and_recursive_ctes
    ...
    base_case = mock_db.select(Sequel.as(1, :level))
  ...
  def test_recursive_cte_with_complex_base_case
    ...
    recursive_case = mock_dataset(:employees)
                     .select(
                       Sequel.qualify(:e, :id),
    ...
    # Since 'org_chart.depth + 1' is a literal, 'org_chart.depth' is not quoted
  ...
  def test_union_queries
    ...
    dataset = dataset1.union(dataset2, from_self: false)
  ...
  def test_union_all_queries
    ...
    expected_sql = "SELECT * FROM (SELECT \"name\" FROM \"users\" UNION ALL SELECT \"name\" FROM \"admins\") AS \"t1\""
  ...
  def test_intersect_queries
    ...
    dataset = dataset1.intersect(dataset2, from_self: false)
  ...
  def test_except_queries
    ...
    expected_sql = "SELECT * FROM (SELECT \"email\" FROM \"users\" EXCEPT SELECT \"email\" FROM \"unsubscribed\") AS \"t1\""
